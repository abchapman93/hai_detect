...
def main():
    ...
    info['num_notes_note_info'] = con.execute("SELECT COUNT (NOTE_ID) FROM note_info").fetchone()[0]
    # How many pats with notes within 30 days?
    info['pats_with_notes_within_30_days'] = con.execute("SELECT COUNT (DISTINCT case_number) FROM note_info\
                                                         WHERE is_within_30_days = 1").fetchone()[0]

    info['notes_within_30_days'] = con.execute("SELECT COUNT (DISTINCT NOTE_ID) FROM note_info\
                                               WHERE is_within_30_days = 1").fetchone()[0]
    info['pats_within_30_days_with_hai'] = con.execute("SELECT COUNT (DISTINCT case_number) FROM note_info\
    ...
    print("Total number of notes in note_info: {num_notes_note_info}".format(**info))
    print('Total number of cases with notes within 30 days of an operation: {pats_with_notes_within_30_days}'.format(**info))
    print("Total number of notes within 30 days: {notes_within_30_days}".format(**info))
    print('Total number of cases with notes within 30 days of an operation with HAI: {pats_within_30_days_with_hai}'.format(**info))
    print("Total number of notes within 30 days with HAI: {notes_within_30_days_with_hai}".format(**info))
    print("Percentage of cases in cohort with HAI: {% of pats in cohort with hai}%".format(**info))
